...
def depth(root, goal, silent=False):
    ...
    return {"result": ans, "steps": ctr, "cost": ans.length()-1, "nodes": open.length()+len(closed)}
...
def hill(root, goal, H, silent=False):
    ...
    while True:
        ...
        if H(best.state()) < H(node.state()):
            ...
            break
    path.reverse()        
    return {"result": path, "steps": ctr, "cost": len(path)-1, "nodes": len(path)-1}
    ...
    while open != []:
        ...
        if goal(node.state()):
            ...
            return {"result": path, "steps": ctr,
                    "cost": len(path)-1, "nodes": len(open)+len(closed)}
        closed.add(node)
        for s in node.successors():
            if s in closed:
                continue;
...
def algA(root, goal, h, silent=False):
    f = lambda p: h(p[0].state()) +len(p)-1
    open = [[root]]
    closed = set()
    ctr = 0
    while open != []:
        ...
        if self.__successors == None:
            ...
            for rule in self.__rules:
                ...
                if self.__legal(child):
                    self.__successors.append(Node(child, self.__rules, self.__legal))
...
def rulemaker(branch):
    def rule(k):
        ...
        def act(n):
            return n.successors()[k]
        return {"pre": pre, "act": act}
...
# graphToNode creates a Node from a GraphNode using rules produced by rulemaker
